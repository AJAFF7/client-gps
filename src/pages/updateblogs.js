...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('https://hama.ajaf.my.id/api/blogs');
        if (!response.data) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  const handleSave = async (postId) => {
    try {
      const modifiedPost = modifiedPosts.find((post) => post._id === postId);
      const response = await axios.patch(`https://hama.ajaf.my.id/api/blogs/${postId}`, modifiedPost);
  ...
  const handleDelete = async (postId) => {
    try {
      ...
      const response = await axios.delete(`https://hama.ajaf.my.id/api/blogs/${postId}`);
