...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('https://hama.ajaf.my.id/api/blogs'); // https://server-kthc.onrender.com/api/blogs
        if (!response.ok) {
  ...
  const handleLike = async (postId) => {
    try {
      const response = await fetch(`https://hama.ajaf.my.id/api/blogs/${postId}/like`, {
        method: 'POST',
      });
      if (!response.ok) {
